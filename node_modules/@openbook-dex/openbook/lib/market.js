...
class Market {
    ...
    async findOpenOrdersAccountsForOwner(connection, ownerAddress, cacheDurationMs = 0, forceSeedAccount = false) {
        ...
        const openOrdersAccountsForOwner = await OpenOrders.findForMarketAndOwner(connection, this.address, ownerAddress, this._programId, forceSeedAccount);
    ...
    async makePlaceOrderTransaction(connection, { owner, payer, side, price, size, orderType = 'limit', clientId, openOrdersAddressKey, openOrdersAccount, feeDiscountPubkey = undefined, selfTradeBehavior = 'decrementTake', maxTs, replaceIfExists = false, }, cacheDurationMs = 0, feeDiscountPubkeyCacheDurationMs = 0) {
        ...
        if (openOrdersAccounts.length === 0) {
            ...
            if (openOrdersAccount) {
                ...
                account = await OpenOrders.getDerivedOOAccountPubkey(ownerAddress, this.address, this.programId);
            }
            transaction.add(await OpenOrders.makeCreateAccountTransaction(connection, this.address, ownerAddress, account.publicKey, this._programId, account.seed));
...
class OpenOrders {
    ...
    static async getDerivedOOAccountPubkey(ownerAddress, marketAddress, programId) {
        const seed = marketAddress.toBase58().slice(0, 32);
        const publicKey = await web3_js_1.PublicKey.createWithSeed(ownerAddress, seed, programId);
    ...
    static async findForMarketAndOwner(connection, marketAddress, ownerAddress, programId, forceSeedAccount = false) {
        // Try loading seed based accounts
        const account = await this.getDerivedOOAccountPubkey(ownerAddress, marketAddress, programId);
    ...
    static async makeCreateAccountTransaction(connection, marketAddress, ownerAddress, newAccountAddress, programId, seed) {
        return web3_js_1.SystemProgram.createAccountWithSeed({
            fromPubkey: ownerAddress,
